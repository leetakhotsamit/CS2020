...
def is_prime_divisor(c, d):
    '''
    Returns a count of the number of prime divisors from a range of 2 to d. If 
    c is divisible than d then a check to see if the number is prime is needed
    to be accounted for.
...
def count_prime_divisors(n):
    '''
    Returns the number of prime divisors given a positive integer n. The 
    variable n can be any number from 0 to 100.
    
    count_prime_divisors(n): Int => Nat
    
    requires: 
    * n is a positive integer
    * n can be any number from 0 to 100
    
    Example
    count_prime_divisors (12) -> 2
    count_prime_divisors(1) -> 0
    '''
...
check.expect("Q4T15",count_prime_divisors(100),2)